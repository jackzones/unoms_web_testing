...
Then /^the serial_number_column exists$/ do
	# sleep 2
expect(on_page(DevicePage).serial_number_column_element.present?).to be true
end

Then /^the oui_column exists$/ do
expect(on_page(DevicePage).oui_column_element.present?).to be true
	# binding.pry
end
...
expect(on_page(DevicePage).iframe_text).to include excepted
end
...
expect(on_page(DevicePage).iframe_text).to include message
end
...
expect(on_page(DevicePage).online?(device_name)).to be_truthy
	# binding.pry
end
...
Then /^设备Reboot成功$/ do
expect(on_page(DevicePage).reboot_success?).to be_truthy
end
...
When /^对设备运行测试菜单下的SetParameterValues任务，参数为'(.+)'，ParameterKey为'(.+)'$/ do |parameter, key|
	visit_page(LoginPage)
	navigate_to(DevicePage).select_device('auto_test0')
	on_page(DevicePage).run_test_spv(parameter, key)
end
